<?php
	...
	function isNull($id, $primer_apellido, $segundo_apellido, $nombre, $user, $pass, $pass_con, $email, $id_departamento){
		if(strlen(trim($id)) < 1 || strlen(trim($primer_apellido)) < 1 || strlen(trim($segundo_apellido)) < 1 || strlen(trim($nombre)) < 1 || strlen(trim($user)) < 1 || strlen(trim($pass)) < 1 || strlen(trim($pass_con)) < 1 || strlen(trim($email)) < 1 || strlen(trim($id_departamento)) < 1)
		{
			return true;
			} else {
			return false;
	...
	function minMax($min=8, $max, $valor){
	...
	function registraUsuario($id, $primer_apellido, $segundo_apellido, $nombre, $usuario, $pass_hash, $email, $id_area, $id_puesto, $activo, $token, $tipo_usuario, $num_pases){
		
		global $mysqli;
		
		$stmt = $mysqli->prepare("INSERT INTO usuarios (id, primer_apellido, segundo_apellido, nombre, password, usuario, correo, id_area, id_puesto, activacion, token, id_tipo, num_pases) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)");
		$stmt->bind_param('issssssiiisii',$id, $primer_apellido, $segundo_apellido, $nombre, $pass_hash, $usuario, $email, $id_area, $id_puesto, $activo, $token, $tipo_usuario, $num_pases);
		$stmt->execute();
	...
	function enviarEmail($email, $nombre, $asunto, $cuerpo){
		...
		$mail->Username = 'carlos.jgt.99@gmail.com'; //Modificar
		$mail->Password = 'copo290317'; //Modificar
		
		$mail->setFrom('carlos.jgt.99@gmail.com', 'Registro de cuenta'); //Modificar
				...
				if($validaPassw){
					...
					$_SESSION['tipo_usuario'] = $id_tipo;
						...
						switch($_SESSION['tipo_usuario']){
							...
							case 2:
								header('Location: Inicioencar.php');
							break;
